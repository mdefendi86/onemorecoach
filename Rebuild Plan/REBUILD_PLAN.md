...
|---|---|---|---|
| `/` | `app/page.tsx` | index.html | Hero · StatsStrip · ProgramsOverview · IncludedGrid · ResultsGrid · TestimonialsGrid · GoogleReviewsBadge (conditional) · **InstagramCtaSection** (placed near the results / social-proof area per §15 #16) · **CoachBioSnippet** (rendered as a "Meet Your Coach" section per §15 #17 — portrait-style headshot + 2–4 sentence first-person bio + link to /about) · ApplyCTA · Footer |
...
| `/in-person` | `app/in-person/page.tsx` | inperson.html | **Decision pending** (open question §15). Either a real page that pulls only the `in-person` slug from `programs.ts`, or a 301 redirect to `/programs#in-person`. Phase 2 renders the real page with a coaching-in-action hero image (sourced from `programs.ts → in-person → heroImageSrc`, per §15 #18). |
| `/about` | `app/about/page.tsx` | about.html | Hero with real headshot + cert badges from `coach.ts` · **First-person** bio paragraphs from `coach.ts` (per §15 #17) · Pull quote · **Coaching-in-action image break** (from `coach.ts → actionShotSrc`, per §15 #18) · HowIHelpYouWin grid · WhyOnlineGrid · Instagram CTA · Bottom CTA |
...
| 17 (new) | Voice / personality — coach-led, first-person | **The site reads as Josh's voice.** The About page is written in first person ("I", "my") so visitors feel like they're meeting the person behind the brand. The brand identity is still **One More Coach** (not "Josh Horton" as a separate brand) — Josh is the owner/coach. The bio in `coach.ts` was rewritten Phase 2 to swap third-person narration ("Josh's journey…") for first person ("My journey…"). The homepage adds a dedicated **"Meet Your Coach"** section above the existing bio snippet so the landing page feels personal without turning into a full bio. Section labels also shift: "The Story" → "My Story"; "Work With Josh" → "Work With Me"; bio-snippet CTA "Full Bio →" → "More About Me →". Reuses the same `coach.ts` and `business.ts` sources of truth — no new brand strings hard-coded in components. | 2026-05-27 |
| 18 (new) | Coaching-in-action imagery distribution | **Use coach + client coaching shots throughout the site to show real coaching, but spread them across pages — never reuse the same shot on multiple pages.** Phase 2 routing: In-Person page hero uses one form-check shot (sourced from `programs.ts → in-person → heroImageSrc`); About page mid-page break uses a different coaching shot (sourced from `coach.ts → actionShotSrc`). Homepage stays on the headshot for the Meet Your Coach section to keep it portrait-led. Coaching shots must **not** appear in any context that implies a before/after transformation result — only the existing `results/result-*.jpg` set is allowed in the Results section. Program/package cards do **not** carry stock gym imagery in Phase 2 (see §15 #19); if a coaching shot doesn't visually match the specific package it's selling, it's not used. | 2026-05-27 |
| 19 (new) | Package-specific imagery — content gap | **The five program packages need their own visually-matching images** before the Programs page can carry meaningful imagery. Phase 2 ships clean text-and-icon program cards rather than forcing mismatched stock gym photos onto them. Specific imagery still needed (tracked in IMAGE_INVENTORY.md's "Content gap: package-specific images" section): (a) Online coaching — remote check-in / app screen / phone-in-hand context; (b) Nutrition coaching — meal prep / portioned food / supplement stack; (c) Training guidance — solo training / form check / programming notebook; (d) Lifestyle coaching — broader "complete program" visual (training + food together); (e) In-person training — coaching at the venue (the existing coaching-in-action set covers this). Once Josh provides these, fields like `program.heroImageSrc` get populated for each program and `ProgramSection` / `ProgramOverviewCard` can render them. Until then, no random gym imagery on cards. | 2026-05-27 |
